...
//	Com_Printf(CON_CHANNEL_SERVER, "------ BSPC MAIN ------\n"); // Outcomment
/*
	bspc.c:465:13: error: 'CON_CHANNEL_SERVER' undeclared (first use in this function)
	  Com_Printf(CON_CHANNEL_SERVER, "------ BSPC MAIN ------\n");
	             ^~~~~~~~~~~~~~~~~~
*/
	...
	if ( argc > 1 && i == argc ) {
		...
		case COMP_BSP2MAP:
		{
			...
			for ( qf = qfiles; qf; qf = qf->next )
			{
				...
				if ( qf->type != QFILETYPE_FF && qf->type != QFILETYPE_BSP ) {
					Warning( "%s is probably not a Fastfile or BSP file\n", qf->origname );
				}
				//
				LoadMapFromBSP( qf );
				//write the map file
